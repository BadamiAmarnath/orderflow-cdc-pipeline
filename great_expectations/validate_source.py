#!/usr/bin/env python3
"""
OrderFlow Great Expectations Source Data Quality Suite
Validates raw/staged data before warehouse loading:
...
logger = logging.getLogger("great_expectations.source_validator")
...
class SourceDataQualityValidator:
    ...
    def validate_products(self) -> Dict[str, Any]:
        ...
        if df.empty:
            return {"success": True, "evaluated": 0, "failures": []}
    ...
    def validate_orders(self) -> Dict[str, Any]:
        ...
        if not df_cust.empty:
            ...
            if not unresolved.empty:
                failures.append(f"ExpectationFailed: expect_column_values_to_be_in_set('customer_id', foreign_key_dim_customers) -> {len(unresolved)} violations: {unresolved[['order_id', 'customer_id']].to_dict(orient='records')}")
    ...
    def run_all_suites(self) -> bool:
        ...
        logger.info("Running Great Expectations Source Data Quality Suites...")
        results = [
        ...
        for r in results:
            if r["success"]:
                logger.info(f" [PASS] Suite '{r['table']}': Evaluated {r['evaluated_rows']} events with 0 errors.")
            else:
                all_success = False
                logger.error(f" [FAIL] Suite '{r['table']}': Found {len(r['failures'])} data quality violations!")
                for f in r["failures"]:
                    logger.error(f"   --> {f}")
